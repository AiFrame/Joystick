...
const int mouseButton_RIGHT = A5;    // input pin for the mouse pushButton
const int mouseButton_MID = 3;    // input pin for the mouse pushButton
const int xAxis = A1;         // joystick X axis  
const int yAxis = A2;         // joystick Y axis
const int zAxis = A4;         // joystick Y axis
...
void loop() {
  ...
  int zReading = -readAxis(zAxis);

  // if the mouse control state is active, move the mouse:
  if (mouseIsActive) {
    Mouse.move(xReading/2, yReading/2, zReading/5);
  }  
  ...
  if (digitalRead(mouseButton_RIGHT) == LOW) {
    // if the mouse is not pressed, press it:
  ...
  if (digitalRead(mouseButton_MID) == LOW) {
    // if the mouse is not pressed, press it:
